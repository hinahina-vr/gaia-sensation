...
const out='artifacts/bi-insights-20260913'; fs.mkdirSync(out,{recursive:true});
...
function check(dataset, number, evidence='保存実データ・代表対象') {
  ...
  assert(!/見直す手がかり|検証する社会課題|NaN|undefined/.test(result.summary),`${dataset.id}: ${result.summary}`);
  assert(result.computedInsight);
  ...
  assert.equal(ai.computedInsight.summary,result.summary);
  assert(statisticsAiPrompt(ai,'概要').user.includes('## インサイト'));
...
for(const def of [{...MARINE_COD_EXHIBIT,unit:'mg/L',periodUnit:'年度',metricLabel:'COD'},...JAPAN_SENSOR_OPEN_EXHIBITS,...JAPAN_POLLUTION_EXHIBITS,...PRTR_BIOLOGY_EXHIBITS]) {
...
for(const key of Object.keys(SOCIAL_PROFILES)) for(const id of ['flat-data','not-enough-comparison','pace-change','different-contexts']) {
  const r=buildSocialInsight({},key,{id}); assert(r.profileKey); assert(r.need);
}
...
try {
  ...
  for(const number of ['10','14','12','28','32','46','67','71']) {
    ...
    assert(summary.includes(discoverData({dataset,rows:dataset.rows}).summary));
    await page.locator('[data-stat-view="findings"]').click();
    assert((await page.locator('.dashboard-reading [data-kind="meaning"]').textContent()).includes(discoverData({dataset,rows:dataset.rows}).summary));
    assert((await page.locator('[data-kind="social-data"]').textContent()).includes('原因・施策効果の検証に追加で必要なデータ'));
    if(number==='10') assert.equal(await page.locator('.dashboard-chart').count(),0,'Country comparisons must not be rendered as a time-series line');
...
const versions=Object.fromEntries(['statistics-decision-insights.js','statistics-social-insights.js','statistics-discovery.js','statistics-lab.js','statistics-refinement.css','statistics-ai.js','src/exploration/annual-statistics.js','src/exploration/food-catalog.js'].map(f=>[f,createHash('sha256').update(fs.readFileSync(f)).digest('hex')]));
...
fs.writeFileSync(`${out}/REPORT.md`, `# BI計算インサイト全展示監査\n\n${checks.length}データセット・系列を確認。保存実データは各展示の代表対象で確認し、全地点×全年の組合せ試験ではありません。LIVE・全球経路の制御入力試験は現在値や実配信の確認ではありません。対象名・期間差・寄与率を計算して提示。原因や介入効果は実証していません。\n\n|展示|対象|指標別プロファイル|確認方法|\n|---|---|---|---|\n${checks.map(c=>`|${c.number}|${c.title.replaceAll('|','／')}|${c.profile}|${c.evidence}|`).join('\n')}\n\n## 全件の読み取り\n\n${checks.map(c=>`### ${c.number} ${c.title}\n\nファクト：${c.fact}\n\nインサイト：${c.insight}\n\n必要データ：${c.need}\n\n制約：${c.limit}\n`).join('\n')}\n\nローカルのみ。AIへの有料送信・本番公開は行っていません。\n`);
