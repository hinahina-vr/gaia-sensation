...
const dir=path.resolve('output/flyers/20260913-yuru');
...
fs.writeFileSync(path.join(dir,'index.html'),`<!doctype html><html lang="ja"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>ゆるキャラ10案</title><style>body{margin:24px;background:#eeeae3;color:#172c36;font:16px/1.5 sans-serif}main{max-width:1500px;margin:auto}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:20px}article{background:white;border-radius:10px;padding:14px}h3{font-size:16px}img{width:100%;height:720px;object-fit:contain}@media(max-width:700px){body{margin:12px}.grid{grid-template-columns:1fr}img{height:auto}}</style><main><h1>惑星の放課後｜ゆるキャラ10案</h1><p>元の紙と青い手描きのデザインを引き継いだ10パターン。縦5・横5。画像を押すと原寸PNGが開きます。</p><p>画像生成で制作したチラシ案です。生成プロンプトは prompts.json に保存しています。</p><section class="grid">${section()}</section></main></html>`);
...
try{
  ...
  if(decoded!==10) throw new Error('Missing image');
  const overflow=[];
  ...
  console.log(JSON.stringify({images:decoded,groups:{yuru:10},horizontalOverflow:overflow}));
